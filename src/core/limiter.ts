...
import { normalizePolicy, type Decision, type FailureMode, type LimiterOptions, type Policy } from './types.js';

/** Redis returns Lua numbers as integers; this narrows the tuple shape. */
type ScriptResult = [number, number, number, number, number];
...
export class RateLimiter {
   ...
   */
  private keyFor(clientId: string, policyName: string, algo: string): string {
    return `${this.prefix}:{${clientId}}:${algo}:${policyName}`;
  }

  async check(clientId: string, policy: Policy, cost = 1): Promise<Decision> {
   ...
   */
  private shouldLease(algorithm: string, burst: number, cost: number): boolean {
    if (!this.leases.enabled || algorithm !== 'token_bucket' || cost < 1) return false;
   ...
   */
  private async checkLeased(
    key: string,
    clientId: string,
    p: ReturnType<typeof normalizePolicy>,
    cost: number,
  ): Promise<Decision> {
    ...
    if (fast === 'hit') return this.leaseDecision(key, clientId, p, nowMs, started, true, 0);
    if (fast === 'denied') {
    ...
    try {
      ...
      switch (p.algorithm) {
        case 'token_bucket': {
          ...
          const ratePerSec = (p.limit * 1000) / p.windowMs;
          res = await this.redis.tokenBucket(
            key,
            String(p.burst),
            String(ratePerSec),
            now,
        ...
        case 'sliding_window_log':
          res = await this.redis.slidingWindowLog(
            key,
            String(p.limit),
            String(p.windowMs),
            now,
        ...
        case 'sliding_window_counter':
          res = await this.redis.slidingWindowCounter(
            key,
            String(p.limit),
            String(p.windowMs),
            now,
...
function shortAlgo(a: string): string {
  if (a === 'token_bucket') return 'tb';
  if (a === 'sliding_window_log') return 'swl';
  return 'swc';
}
