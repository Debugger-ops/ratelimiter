...
import { MetricsReader, type MetricsRecorder } from '../metrics/metrics.js';
import { validatePolicy, type Policy } from '../core/types.js';
...
export function adminRouter(deps: AdminDeps): Router {
  ...
  router.use((req, res, next) => {
    ...
    if (req.method === 'GET' && req.path === '/stream') return next();
    if (!adminToken) return next(); // dev mode; server logs a warning at boot
  ...
  router.put('/policies/:name', async (req: Request, res: Response) => {
    const body = { ...(req.body as Partial<Policy>), name: req.params.name };
    try {
      validatePolicy(body);
  ...
  router.put('/assignments/:clientId', async (req, res) => {
    const policy = (req.body as { policy?: string })?.policy;
...
async function snapshot(deps: AdminDeps, reader: MetricsReader, seconds: number) {
