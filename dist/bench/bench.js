...
async function benchCore() {
    ...
    console.log('algorithm'.padEnd(24) +
    ...
    for (const [name, policy] of Object.entries(POLICIES)) {
        ...
        for (let i = 0; i < 500; i++)
            await limiter.check(`warm-${i % 50}`, policy);
...
async function benchLease() {
    ...
    const run = async (label, leaseOn) => {
        ...
        for (let i = 0; i < 500; i++)
            await limiter.check(client, policy); // warm
    ...
    console.log('─'.repeat(90));
    console.log(`${(on.rps / off.rps).toFixed(1)}x throughput · ` +
...
async function main() {
    await benchCore();
    if (!process.argv.includes('--no-lease'))
        await benchLease();
