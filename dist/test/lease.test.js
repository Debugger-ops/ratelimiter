 ...
 */
function leased(redis, lease = {}, over = {}) {
...
describe('leases do not loosen the limit', () => {
    it('admits exactly the burst from 8 leasing instances at once', async () => {
        ...
        const results = await Promise.all(Array.from({ length: 500 }, (_, i) => fleet[i % CONNECTIONS].check(c, policy)));
    ...
    it('a leased fleet and an unleased fleet admit the same number', async () => {
        ...
        const plainFleet = clients.map((r) => new RateLimiter(r, { keyPrefix: 'par-p', now: () => clock.now }));
        const run = async (fleet) => {
            const c = id();
            const out = await Promise.all(Array.from({ length: 400 }, (_, i) => fleet[i % CONNECTIONS].check(c, policy)));
    ...
    it('charges a multi-token cost against the lease, not once per lease', async () => {
        ...
        let allowed = 0;
        for (let i = 0; i < 30; i++) {
            if ((await limiter.check(c, policy, 5)).allowed)
                allowed++;
        }
...
describe('what is deliberately not leased', () => {
    it('never leases a sliding window, so `login` is excluded by construction', async () => {
        const { limiter } = leased(clients[0]);
        const c = id();
        const login = { name: 'login', algorithm: 'sliding_window_log', limit: 5, windowMs: 300_000 };
        for (let i = 0; i < 5; i++) {
...
describe('leases under a Redis outage', () => {
    ...
    it('makes a fail-closed outage bounded rather than a cliff', async () => {
        ...
        for (let i = 0; i < held + 5; i++) {
            if ((await limiter.check(c, policy)).allowed)
                served++;
        }
...
describe('through the HTTP middleware', () => {
    it('serves a burst over HTTP from one lease and labels the source header', async () => {
        ...
        const app = express();
        app.get('/x', rateLimit({ limiter, policies, metrics }, { policy: 'lease-http' }), (_req, res) => res.json({ ok: true }));
        ...
        for (let i = 0; i < 25; i++) {
            ...
            codes.push(r.status);
            if (r.headers.get('x-ratelimit-source') === 'lease')
                leaseServed++;
        }
...
describe('the throttled path', () => {
    it('refuses a flood locally instead of one round trip per rejected request', async () => {
        ...
        let drained = 0;
        while (drained < 200 && (await limiter.check(c, policy)).allowed)
            drained++;
...
describe('lease bookkeeping', () => {
    ...
    it('bounds its own memory, returning the tokens it evicts', async () => {
        ...
        for (let i = 0; i < 40; i++)
            await limiter.check(`evict-${i}`, policy);
        const s = limiter.leases.stats();
