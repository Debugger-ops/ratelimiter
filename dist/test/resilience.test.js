...
const policy = { name: 'res', algorithm: 'token_bucket', limit: 10, windowMs: 60_000, burst: 10 };
...
describe('policy store', () => {
    ...
    it('rejects nonsense before it can reach the hot path', async () => {
        const store = new PolicyStore(live);
        await store.init();
        await expect(store.upsert({ name: 'bad', algorithm: 'magic', limit: 1, windowMs: 1 })).rejects.toThrow();
        ...
        await expect(store.upsert({ name: 'huge', algorithm: 'sliding_window_log', limit: 1_000_000, windowMs: 60_000 })).rejects.toThrow(/memory/);
