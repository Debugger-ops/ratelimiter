...
const log = { name: 'swl-test', algorithm: 'sliding_window_log', limit: 5, windowMs: 10_000 };
const counter = { name: 'swc-test', algorithm: 'sliding_window_counter', limit: 100, windowMs: 60_000 };
const fixedish = { name: 'swc-small', algorithm: 'sliding_window_counter', limit: 10, windowMs: 10_000 };
describe('sliding window log (exact)', () => {
    ...
    it('refuses the boundary burst a fixed window would allow', async () => {
        ...
        for (let i = 0; i < 5; i++)
            await f.limiter.check(c, log);
        f.advance(1); // "new window" in a fixed-counter design
        let allowed = 0;
        for (let i = 0; i < 5; i++)
            if ((await f.limiter.check(c, log)).allowed)
                allowed++;
...
describe('sliding window counter (approximate, O(1) memory)', () => {
    it('allows exactly `limit` inside one window', async () => {
        ...
        let allowed = 0;
        for (let i = 0; i < 150; i++)
            if ((await f.limiter.check(c, counter)).allowed)
                allowed++;
    ...
    it('weights the previous window down as the current one advances', async () => {
        ...
        for (let i = 0; i < 10; i++)
            expect((await f.limiter.check(c, fixedish)).allowed).toBe(true);
        ...
        let allowed = 0;
        for (let i = 0; i < 10; i++)
            if ((await f.limiter.check(c, fixedish)).allowed)
                allowed++;
    ...
    it('costs two keys per client no matter how large the limit is', async () => {
        const c = id();
        for (let i = 0; i < 100; i++)
            await f.limiter.check(c, counter);
        const keys = await f.redis.keys(`*{${c}}*`);
    ...
    it('stays within a bounded error of the exact algorithm, at every phase', async () => {
        ...
        const STEP = 500;
        const PHASES = 8;
        const run = async (algorithm, startAt) => {
            const c = id();
            const p = { name: `acc-${algorithm}`, algorithm, limit: 60, windowMs: WINDOW };
            f.clock.now = startAt;
            let allowed = 0;
            for (let i = 0; i < TRACE; i++) {
                if ((await f.limiter.check(c, p)).allowed)
                    allowed++;
                f.advance(STEP);
            }
            return allowed;
...
//# sourceMappingURL=sliding-window.test.js.map