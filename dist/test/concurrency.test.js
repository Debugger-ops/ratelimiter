...
const limiters = clients.map((r) => new RateLimiter(r, { keyPrefix: 'conc', now: () => frozen }));
beforeAll(async () => {
...
async function stampede(policy, attempts) {
    const c = id();
    const results = await Promise.all(Array.from({ length: attempts }, (_, i) => limiters[i % CONNECTIONS].check(c, policy)));
...
describe('atomicity under a stampede', () => {
    it('token bucket admits exactly the burst, from 8 connections at once', async () => {
        const allowed = await stampede({ name: 'c-tb', algorithm: 'token_bucket', limit: 50, windowMs: 60_000, burst: 50 }, 500);
        expect(allowed).toBe(50);
    });
    it('sliding window log admits exactly the limit', async () => {
        const allowed = await stampede({ name: 'c-swl', algorithm: 'sliding_window_log', limit: 50, windowMs: 60_000 }, 500);
        expect(allowed).toBe(50);
    });
    it('sliding window counter admits exactly the limit', async () => {
        const allowed = await stampede({ name: 'c-swc', algorithm: 'sliding_window_counter', limit: 50, windowMs: 60_000 }, 500);
        expect(allowed).toBe(50);
    });
    it('holds with a multi-token cost, where a partial spend would be visible', async () => {
        const allowed = await stampede({ name: 'c-cost', algorithm: 'token_bucket', limit: 100, windowMs: 60_000, burst: 100 }, 200);
