...
async function main() {
    ...
    app.get('/metrics', async (_req, res) => {
        ...
        if (lease.enabled) {
            lines.push('# HELP ratelimit_lease_decisions_total Decisions taken on the leased path.', '# TYPE ratelimit_lease_decisions_total counter', `ratelimit_lease_decisions_total ${lease.decisions}`, '# HELP ratelimit_lease_acquires_total Redis round trips spent refilling leases.', '# TYPE ratelimit_lease_acquires_total counter', `ratelimit_lease_acquires_total ${lease.acquires}`, '# HELP ratelimit_lease_redis_ops_saved_total Round trips the lease layer removed.', '# TYPE ratelimit_lease_redis_ops_saved_total counter', `ratelimit_lease_redis_ops_saved_total ${lease.redisOpsSaved}`, '# HELP ratelimit_lease_tokens_returned_total Unspent leased tokens handed back.', '# TYPE ratelimit_lease_tokens_returned_total counter', `ratelimit_lease_tokens_returned_total ${lease.tokensReturned}`, '# HELP ratelimit_lease_entries Clients currently holding a lease on this instance.', '# TYPE ratelimit_lease_entries gauge', `ratelimit_lease_entries ${lease.entries}`);
        }
        lines.push('# HELP ratelimit_decision_latency_ms Middleware decision latency.', '# TYPE ratelimit_decision_latency_ms summary', `ratelimit_decision_latency_ms{quantile="0.5"} ${l.p50Ms}`, `ratelimit_decision_latency_ms{quantile="0.95"} ${l.p95Ms}`, `ratelimit_decision_latency_ms{quantile="0.99"} ${l.p99Ms}`, `ratelimit_decision_latency_ms_count ${l.count}`, '');
...
const escapeLabel = (s) => s.replace(/\\/g, '\\\\').replace(/"/g, '\\"').replace(/\n/g, ' ');
main().catch((err) => {
