...
describe('token bucket', () => {
  ...
  it('refills continuously, not in steps', async () => {
    const c = id();
    for (let i = 0; i < 10; i++) await f.limiter.check(c, policy);
  ...
  it('never refills past capacity, however long the client idles', async () => {
    ...
    let allowed = 0;
    for (let i = 0; i < 50; i++) if ((await f.limiter.check(c, policy)).allowed) allowed++;
  ...
  it('keeps clients independent', async () => {
    const a = id();
    const b = id();
    for (let i = 0; i < 10; i++) await f.limiter.check(a, policy);
