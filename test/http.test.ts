...
const get = (path: string, key: string) =>
  fetch(`${base}${path}`, { headers: { 'x-api-key': key } });
...
describe('express middleware', () => {
  ...
  it('separates clients by API key', async () => {
    const a = id();
    const b = id();
    for (let i = 0; i < 3; i++) await get('/open', a);
  ...
  it('lets skipped routes through without touching Redis', async () => {
    const key = id();
    for (let i = 0; i < 10; i++) expect((await get('/skipped', key)).status).toBe(200);
    expect((await get('/open', key)).status).toBe(200); // quota untouched
  });

  it('records both verdicts in the metrics series', async () => {
    const key = id();
    for (let i = 0; i < 5; i++) await get('/open', key); // 3 allowed, 2 throttled
    await metrics.flush();
  ...
  it('resetting a client clears its limiter state', async () => {
    const key = id();
    for (let i = 0; i < 3; i++) await get('/open', key);
    expect((await get('/open', key)).status).toBe(429);
