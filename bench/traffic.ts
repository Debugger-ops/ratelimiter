...
interface Sim {
  ...
  burst?: { every: number; size: number };
  path?: string;
}
...
const counts = new Map<string, { ok: number; limited: number; err: number }>();

function tally(key: string, status: number) {
  const row = counts.get(key) ?? { ok: 0, limited: 0, err: 0 };
  if (status === 200) row.ok++;
  else if (status === 429) row.limited++;
  else row.err++;
  counts.set(key, row);
}

async function hit(sim: Sim) {
...
async function main() {
  ...
  for (const sim of SIMS) {
    ...
    if (sim.burst) {
      timers.push(
        setInterval(() => {
          for (let i = 0; i < sim.burst!.size; i++) void hit(sim);
  ...
  setTimeout(() => {
    ...
    for (const [k, v] of counts) {
      const total = v.ok + v.limited;
      console.log(
        `  ${k.padEnd(18)} allowed ${String(v.ok).padStart(5)}  throttled ${String(v.limited).padStart(5)}  ` +
